...
### 풀이
<pre>맵을 2배로 확장해야 한다
-> 맵 탐색을 할때 선이 아닌 좌표를 기준으로 하기 때문에 첫번째 테스트 케이스에서 y좌표가 5일때 y=6인 좌표로 이동할 수 없지만 좌표 기준으로 하면 같은 테두리이기때문에 이동할 수 있는 것처럼 보인다. 따라서 좌표를 2배로 늘림으로써 좌표들이 맞다아있지만 실제로는 이동할수 없는 좌표들사이의 이동을 방지한다

풀이 1
사각형 영역을 -1로 모두 채우면서 주어진 좌표를 list에도 저장한다
bfs로 탐색을 하면서 road의 값이 0보다 작고 isBoundary안에 속하면 탐색을 계속한다 이때 탐색이 가능한 경로라면 road의 값을 경로+1을 해주어 방문표시처럼 체크해준다 따라서 방문표시 배열은 따로 필요가 없다 
IsBoundary 함수는 사각형의 값이 내부인지 테두리인지 파악하는 함수이다. list에 사각형의 시작좌표와 끝좌표를 저장해 두고 시작좌표보다 크고 끝좌표보다 작으면 내부의 값임을 알 수 있다.

풀이 2
사각형 영역을 전부 칠한다음 테두리 값을 제외하고 내부의 값을 모두 지워 테두리만 남긴다
그리고 bfs로 탐색을 한다 
여기서는 테두리인지 내부의 값인지 이미 판단을 다 한 상태에서 탐색을 하기 때문에(1인지 아닌지) 바로 road의 배열에 거리를 저장할 수 없다 그래서 visited으로 방문 체크 배열을 따로 만들어서 탐색한다</pre>
