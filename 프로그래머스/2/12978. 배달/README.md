...
### 풀이
<pre>Bfs 로 풀려고 했으나 bfs는 모든 간선의 비용이 동일할 때만 최단거리를 구할 수 있다.
따라서 가중치가있는 그래프에서는 다익스트라(Dijkstra)알고리즘을 써야한다.
-> 가장 짧은 경로를 먼저 확정해 나가는 방식

그래프 노드 클래스를 만든다.
인접리스트를 생성한다
양방향 간선을 저장
거리배열 생성 1번마을에서 시작하므로 1은 0을 저장
우선순위 큐 사용
먼저 1번마을, 거리 0을 대입한다 
cost의 길이가 거리배열에 저장된 숫자 보다 크면 루프를 계속한다.
아니라면 마을을 하나씩 꺼내면서 새로운 거리를 저장한다.</pre>
