...
<pre>5개의 대기실 중에서 한개의 대기실씩 탐색을 한다
P와 다른 P사이의 거리가 탐색 대상이므로 2차원 배열을 돌며 P를 만나면 BFS로 탐색을 시작한다

이때 탐색 조건은 상하좌우를 움직이며 다른 P가 있는지 탐색하는 것이다
BFS로 탐색을 하면서 
- 인덱스가 범위를 벗어나거나
- 이미 방문한 곳이거나
- 파티션(‘X’)이거나
- 거리가 2이상이면 다시 루프를 돌며 탐색한다.
그리고 
- P를 만났는데 거리가 1이면 실패로 0을 반환한다
- P를 만났고 거리가 2이라면
  - 직선 거리에 있다면 실패 
  - 대각선 거리에 있다면 대각선으로 만나러 가는길이 모두 파티션(‘X’)이여야만 성공이다
-위의 조건을 모두 만족하고 현재 값이 ‘O’라면 큐에 삽입한다.
이때 O인 조건을 확인하는 이유는 BFS는 이동 가능한 칸만 큐에 넣어야 하기 때문이다. P는 발견하면 판정하는 대상이지 탐색할 대상이 아니다.

=> 중요한 점은 P끼리의 비교가 아니라 P에서 빈자리만을 따라서 거리 2 이내에 P가 있는지 확인하는 문제 이다.</pre>
