...
### 풀이
<pre>풀이 1 정규식 이용하기
각 문자열을 하나씩 검사해서 순서가 어긋나지 않았는가만 확인하면 됨
따라서 각 스킬트리마다 필요한 스킬만 남기고 순서를 확인하면 끝입니다. 

풀이 2 직접 비교하기


**참고 replaceAll()이란 String 메서드로 첫번째 인자로 정규식을 받고, 두 번째 인자로 바꿀 문자열을 받습니다. -> "문자열".replaceAll("패턴", "대체문자")
정규식 “[^” + skill + "]" : skill문자열에 포함되지 않는 문자 ^가 부정을 뜻합니다.

startsWith() :  String 메서드로 이 문자열이 특정 문자열로 시작하는지 확인 하는 함수
indexOf() : String 안에서 특정 문자가 어디에 있는지 찾아주는 메서드</pre>
