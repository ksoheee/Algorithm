...
### 풀이 
<pre>1. 대각선이 지나는 단위정사각형의 공식: (W+H)−(gcd(W,H))
m×n 사각형에서
m과 n의 최대공약수가 a라고 할 때
m = ab
n = ac라고 합시다. (b,c는 당연히 서로소)
...
예를 들어 (0,0), (8,12)라고 한다면 y = 12/8x 이다. 이때 x좌표는 1부터 w길이 미만이다.</p>
<p>x=1 (1.15) 일때 멀쩡한 칸 1 개
x=2 (2,3) 일때 멀쩡한 칸 2개
x=3 (3.4.5) 일때 멀쩡한 칸 3개
…
x=7 (7. 10.5) 일때 멀쩡한 칸 10개
이 계산은 직사각형의 대각선을 그은 반쪽 삼각형에만 해당하므로 *2를 해주면 된다.</pre>
