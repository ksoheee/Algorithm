...
### 풀이
<p>광물은 5개씩 한 덩어리로 나누어 생각한다.
피로도를 최소화하기 위해, 각 구간(5개 묶음)의 돌 곡괭이 기준 피로도를 계산한 뒤,
피로도가 높은 순서대로 정렬한다.</p>
<p>이후 곡괭이는 다이아 → 철 → 돌 순서로 배정하여
가장 피로도가 높은 구간에 좋은 곡괭이를 우선적으로 사용한다.</p>
<p>ArrayList를 사용하는 이유는,
각 그룹은 [다이아 개수, 철 개수, 돌 개수]로 열의 구조는 고정되어 있지만,
행(그룹)의 개수는 가변적이기 때문이다.</p>
<p>행의 개수가 가변적인 이유는,
곡괭이의 수가 광물의 개수보다 적을 수도 있어
모든 광물을 그룹화할 필요가 없기 때문이다.</p>
