...
1부터 증가하는 rows×columns 크기의 배열, 최솟값 들을 저장할 배열이 필요하다.

행렬을 회전하는 건 
회전해야할 행 첫번째 좌표의 값을 temp에 저장해 두고 아래처럼 회전을 한다.
1. 왼쪽 열 위-> 아래
2. 아래 행 왼->오
3. 오른쪽 열 아래 -> 위
4. 위의 행 왼-> 오
1,2,3,4번으로 회전하면서 각각 최소값을 찾아준다.
회전이 끝났다면 첫번째 행, 두번째 열 값에는 temp에 저장한 값을 넣어준다.</pre>
