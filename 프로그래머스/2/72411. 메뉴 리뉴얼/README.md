...
### 풀이
<pre>채택될 메뉴의 개수를 모르니 ArrayList로 선언해 준다.

코스요리 별로 최대 선택 횟수를 저장할 배열을 선언한다.

가능한 모든 메뉴의 조합과 선택 횟수를 저장할 맵을 선언한다.

각 손님이 주문한 메뉴는 알파벳순으로 정렬한다.
Course.length의 수 만큼 corse[j]개의 조합으로 만들 수 있는 모든 조합을 찾는다.

코스 찾는 함수
- 선택할 문자의 위치
- 현재까지 선택된 메뉴의 개수
- 조합된 코스(문자열)
- coruse[j]의 수 (코스로 선택해야 하는 메뉴의 수)

coruse[j](코스로 선택해야하는 메뉴의 수)와 현재까지 선택된 메뉴의 수가 같다면
맵에 조합을 저장한다
이때 맵안에 이미 그 조합의 문자열(키)가 값(value)에 +1을 해주고, 처음으로 저장한다면 1을 저장해준다.
그리고 그 조합의 값(조합이 선택된 횟수)와 max값을 비교하여 갱신한다.
coruse[j](코스로 선택해야하는 메뉴의 수)와 현재까지 선택된 메뉴의 수가 같지 않다면
한 사람이 주문한 요리의 수 만큼 반복하면서 재귀로 함수를 호출한다.
매번 str 문자열(한사람이 주문한 요리수)에서 i를 하나 선택하고 s 문자열에 붙인다.
I+1 부터 시작하게 하고 재귀함수를 호출하면 이전에 선택한 문자들보다 뒤의 문자들만 탐색하게 된다. 

메뉴 맵에 적힌 모든 경우의 수 중에서 아래의 모든 것을 만족할때 answeradml List에 담는다
- 코스가 course[I]개 이고
- 2번이상 선택된 조합
- course[I]개 선택한 메뉴의 조합중에서 가장 많이 선택된것

리스트를 배열로 바꾸고 사전순 정렬하여 출력한다.

* String -> char array : toCharArray()
* char array -> String : String.copyValueOf(char array대입)

참고: <a href="링크주소">https://jisunshine.tistory.com/146</a>
</pre>
