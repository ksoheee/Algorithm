...
### 풀이
<pre><b>풀이 1</b>
현재 필요한 서버와 현재 사용간으한 서버를 구하고 두 개의 차이를 구하면 증설해야하는 서버의 수를 알 수 있다. 
현재 인원 수 기준으로 필요한 서버 수 계산 : player[I]/m 
현재 사용가능한 서버의 개수 : 
한번 증설하면 k 시간 동안 유지된다
따라서 i-k+1시각 부터 i-1시각까지 증설된 서버들의 합이다.
server[I] = I 시각에 새로 추가한 서버의 개수를 의미한다.

증설 서버 수 계산 : 현재 필요한 서버수 - 현재 사용가능한 서버 수 
이 값이 양수면 그만큼 서버를 새로 증설한다. 
...
만료된 서버 제거 
큐안의 만료시간과 현재 시각이 같아지면 큐에서 꺼낸다. 그리고 현재 서버수에서도 빼준다.
만료가 여러대가 될 수 있으니 반복문(while)로 처리한다. 

현재 필요한 서버 수 계산
현재 필요한 서버수 = players[i] / m

현재 서버와 필요한 서버 비교
현재 서버에서 필요한 서버를 빼주고 
양수 값이라면 서버가 추가로 필요 없다는 것이고
음수 값이라면 서버가 추가로 필요하다는 뜻 현재서버에서 필요한 서버를 빼준 만큼 증설해줘야한다.
서버수를 늘리고, 증설횟수도 증가 시켜주고, 증설한 서버는 큐에 대입한다. 
</pre>
