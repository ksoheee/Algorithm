...
### 풀이
<pre>풀이 1번
1. 임의의노드에서 가장 먼 노드 찾기
2. 그 노드에서 가장 먼 거리 찾기
임의의 노드 S에서 가장 먼 노드 X는 반드시 트리의 지름의 끝점 중 하나이다.
그 이유는 지름을 이루는 두 끝점을 A와 B라고 한다면, A————B 의 사이의 임의의 노드 S를 찍는다면 S에서 가장 먼 거리는 S-A 사이의 거리 또는 S-B 사이의 거리가 되기 때문이다.

풀이 2번
모든 노드 u를 한 번씩 ‘중간점’으로 가정하고,
u에서 서로 다른 가지로 내려가는 가장 긴 두 경로를 더한 값을 지름 후보로 둔다.
이 후보들 중 최댓값이 트리의 지름이다.</pre>
