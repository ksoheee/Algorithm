...
### 풀이
<pre>공통 부분 문자열은 두 문자열에서 연속으로 같은 부분이다. 현재 위치에서 문자가 같다면 이전 문자들도 연속으로 같았는지가 중요하다 
 -> 이전 문자가 같으면 +1 이때 i또는 j가 0이라면 공통 부분 문자열이 길이가 1이므로 dp배열에 1을 저장한다</pre>
