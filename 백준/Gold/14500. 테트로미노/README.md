...
### 풀이
<pre>Bfs/dfs로만 풀수 있음 
다만 dfs/bfs로 만들 수 없는 ㅏ,ㅓ,ㅗ,ㅜ 모양만 예외처리해주면 됨.
그리고 이문제는 dfs가 더 효율적 깊이 제한이 명확하기 때문에! 
슬라이딩 윈도우는 보통 연속된 구간 합(부분 합)문제에 쓰인다

예외처리
중심 값을 구하고 상하좌우를 이동하며 3방향만 값을 합치는 방식으로 구한다. </pre>
