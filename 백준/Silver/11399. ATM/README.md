...
### 풀이
<p>배열을 오름차순으로 정렬하고 앞에서부터 더하는 전략이 매 순간 최적의 선택을 한 것이고 이게 전체적으로도 최적의 해를 보장 (정렬+ 누적합을 이용하는 방식이 그리디의 전형적인 패턴)</p>
<p>정렬되어 있는 배열, 이전 까지 대기 시간 변수, 각 사람 별 대기 시간의 총 합 변수가 필요하다. </p>
<p>이때 정렬 방법은 두가지가 있다.</p>
<p>1. Arrays.sort()</p>
<p>2. Counting Sort()</p>
