...
2. 점화식 수정
즉시 얻는 즐거움(I *(n-1))과 이후 얻는 즐거움을 합해야함(dp[i]+dp[n-1] )
-> dp[n] = max(dp[i]+dp[n-1] +i*(n-i))
그러나 N이 10억크기이면 배열을 10억크기로 만들수도 없고 순회하면 시간도 너무 오래걸림
...
dp[3]=3
dp[4]=6
2와 2로 쪼개면: 2×2+dp[2]+dp[2]=4+1+1=6
dp[5]=10
2와 3으로 쪼개면:2×3+dp[2]+dp[3]=6+1+3=10
=> 삼각수 인걸 알 수 있다.

삼각수는 1부터 어떤 수 n까지의 자여수 합을 말하는데
공식은 n(n+1)/2이다. 

이때 피자의 쌍의 수는 항상 N개 중에 2개 고르기라서 N-1까지만 합해지므로 
n(n-1)/2를 적용해서 풀 수 있다.</pre>
