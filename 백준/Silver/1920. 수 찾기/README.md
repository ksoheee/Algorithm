...
### 풀이
<pre>배열은 반드시 정렬되어 있어야 한다!
타겟 값이 탐색 해야할 값들의 중간 값 보다 크다면 left = mid+1
타겟 값이 탐색 해야할 값들의 중간 값 보다 작다면 right = mid-1
값이 같다면 탐색 성공! 
left가 right보다 커졌다면 탐색에 실패한 것으로 반복문을 종료한다.</pre>
