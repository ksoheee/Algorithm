...
### 풀이
<pre>상하좌우로 움직일 수 있다고 했으니 상하좌우를 이동할 배열
방문체크 배열
인접행렬을 담을 배열이 필요하다

모든 길을 탐색하면서 값이 1이고 방문하지 않았다면 dfs로 음식물을 탐색하는데

dfs에서는 먼저 입력 받은 x,y좌표를 방문표시하고,
상하좌우로 움직이면서x,y의 좌표가 
- 행렬의 범위를 벗어나지 않고
- 방문하지 않았고
- 음식물이 있는(1)이라면 
dfs를 호출한다. 
이때 음식물의 size를 재야하므로 재귀함수에서 재귀함수가 호출될 때마다 size를 늘려준다.</pre>
