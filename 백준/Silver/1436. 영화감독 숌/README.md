...
### 풀이 1
<p>첫 번째 시작 숫자가 666이므로 666부터 계속 증가하다가 666이 들어가 있는 숫자라면 count값을 증가시킨다.</p>
<p>count값과 입력 받은 값이 같다면 그 수 출력</p>
<p>666이 포함 되어있는지 확인은 int를 String으로 변환한 뒤 contains()메서드를 사용한다. 문자열이 있다면 true, 없다면 false를 반환한다.</p>
