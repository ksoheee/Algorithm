...
### 풀이
<pre>현재 계단에서 가질 수 있는 최대 점수는 이전에 어떤 계단을 밟았는지에 따라 달라짐
그 전의 선택이 다음 값에 영향을 미친다 => dp
[상태]
dp[i] = i 번째 계단을 밟았을때 최대 점수
...
I-1 <- I-3 
I-3에서 두칸 점프 
dp[i-3] + dp[i-1]+ arr[i]이 아니고 dp[i-3] + arr[i-1] + arr[i]인 이유는 dp[i-1]을 사용하면 안 되는 이유는 dp[i-1]이 이미 dp[i-3]까지의 점수를 포함하는 누적 값이기 때문이다.
따라서 dp[i-3]과 dp[i-1]을 더하면 같은 구간이 중복된다.
그래서 arr[i-1]만 ‘새로 밟은 계단 점수’로 더하는 것이다.

[초기값]
dp[0]=0
dp[1] = 1번째 입력값
dp[2] = 1번째 입력값+  2번째 입력값
dp[3] = max(1번째 입력값 , 2번째 입력값) + 3번째 입력값

[시간복잡도]
계단의 수 n
각 상태를 계산할때 보는 이전 상태 2개
=>O(n)</pre>
