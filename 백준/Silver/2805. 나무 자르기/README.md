...
### 출력 
 ...
 <pre><b>이분탐색</b>
이분탐색 범위를 설정한다.
left = 0, right = max(tree)
자르는 위치(중간값)를 구한다. 
나무의 잘린 길이 = 나무의 높이 - 자르는위치(중간값)
잘린 나무들의 합을 구한다. 이때 자르는 위치가 나무의 높이보다 크다면 합산하지 않는다.

자른 나무들의 길이의 합이 M 보다 크다면 더 높이 잘라도 된다는 뜻으로 자르는 위치의 높이를 높여야 한다. 즉 하한값을 높인다.
반대로 자른 나무들의 길이의 합이 M 보다 작다면 잘린 나무가 부족하므로 자르는 위치의 높이를 낮춰야 한다. 즉 상한값을 낮춘다.
 </pre>
