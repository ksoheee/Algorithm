...
### 풀이 
<p>세 카드의 합이 M이랑 같이 않는 이상 모든 경우의 수를 탐색한다. 즉 최악의 경우 수행시간이 O(N<sup>3</sup>) 이 된다.</p>
<p>조건에 세 카드의 합이 M을 넘으면 안된다는 조건이 있다. 그렇다면 카드 한장이 M보다 크면 탐색할 필요가 없다. </p>
<P>또한 첫 번째를 고르고 두 번째 카드를 선택했을 때 합이 M보다 크면 탐색할 가치가 없다. </P>
