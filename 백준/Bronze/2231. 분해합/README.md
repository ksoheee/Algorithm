...
### 출력 
 ...
 ### 풀이 1
 <p>1부터 입력 받은 N까지 한 개씩 모두 대입해 보는 것</p>
 
 ### 풀이 2
<P>N(4)=K+K1+K2+K3+K4</P>
<p>N(4)-(K1+K2+K3+K4)=K</p>
<p>즉, 네자릿수 N의 각 자릿수 합이 최대일때는 9+9+9+9 일 것이다. 우리는 입력받은 정수 N에 대하여 자릿수의 길이만큼 9를 빼주면 된다. 그 미만의 생성자가 될 수 없다는 것이다.</p>
<p>정리하면 N-(9xK의 길이)부터 탐색하여 N까지만 탐색하면 된다. </p>
